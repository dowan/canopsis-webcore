...
define([
    'canopsis/brickloader/lib/jquery/dist/jquery'
], function() {
    ...
    var i18n = {
        ...
        _: function(word, noDeprecation) {
            ...
            if(window.Ember && noDeprecation === undefined) {
                Ember.deprecate('You should not use i18n tools directly when ember is loaded. Please consider using Ember.String.loc instead. ', !window.canopsisConfiguration.EmberIsLoaded);
            }
        ...
        showTranslation: function (word, isTranslated) {
            if (window.canopsisConfiguration.SHOW_TRANSLATIONS) {
                if(isTranslated) {
        ...
        downloadDefinitions: function () {
            ...
            if (window.canopsisConfiguration.DEBUG && window.canopsisConfiguration.TRANSLATE) {
                $.ajax({
                    url: '/rest/object/i18n',
                    success: function(data) {
