...
def load_translations(lang='en'):
    ...
    if isfile(lang_file):
        try:
            ...
            logger.error(
                'Unable to load po file for language {} : {}. file location:'.
                format(lang, e, lang_file))
            ...
            if as_attachment:
                try:
                    response.headers['Content-Disposition'] = \
                        'attachment; filename="%s"' % file_name.encode("utf8")
                except Exception as err:
                    logger.error(err)
...
def echo():
    try:
        ...
        params = {'content': ['error while parsing'], 'header': [],
        'filename': []}
    ...
    response.headers["Content-Disposition"] = "attachment; filename=" + \
        params.get('filename', 'file.ext')[0]
...
def add_file():
    ...
    A json in a string will be return to avoid Bottle to automatically set
        header to json
    """
    ...
    if data.filename and data.file:
        if allowed_mimetypes.get(data.filename.split('.')[-1], False):
            ...
            cfile_record.put_data(
                data.file.read(), file_name=data.filename,
                content_type=content_type)
            try:
                file_id = storage.put(cfile_record)
                data = {'success': True, 'data':
                {'code': 200, 'message': 'File uploaded',
                'filename': data.filename, 'file_id': str(file_id)}}
            except Exception as err:
                data = {'success': False,
                'data': {'code': 500, 'message': err}}
        else:
            data = {'success': False,
            'data': {'code': 415, 'message': 'Unsupported Media Type'}}
    else:
        data = {'success': False,
...
def update_file(metaId=None):
    ...
    try:
        ...
        logger.error(
            "Error when updating report %s: %s" % (metaId, err))
